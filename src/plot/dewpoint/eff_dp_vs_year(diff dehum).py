...
def plot_eff_hum_vs_time(loc, show, write):
    ...
    ambient['Dewpoint(F)'] = pd.to_numeric(ambient['Dewpoint(F)'], errors='coerce')
    ...
    start_date = pd.to_datetime(f'2023-05-01')
    ...
    ax1.plot(ambient["Time"], ambient["Dewpoint(F)"], '-', label="Dewpoint(F)", color="tab:purple")
    ax1.set_xlabel("Month")
    ax1.set_ylabel("Dewpoint(F)", color="tab:blue")

    # Plot Dewpoint on the second axis
    ax2.set_ylim(0,2)
    ...
    # Add vertical line when dehumidifier was replaced
    ...
    # Set plot title
    plt.title(f"Dewpoint(F) and Efficiency [L/kWh] Vs. Time ({loc}) (May To November 2023) (diff dehum analysis)")
    
    # Save the plot if requested
    if write == 'Y':
        plot_filename = fr'C:\Users\ajayj\DehumGraph\plots\Effic_DP_Vs_Time ({loc}) (May_Nov_2023) (diff dehum analysis).png'
...
if __name__ == '__main__':
    loc = input("Enter a location to plot Effic and D.P. Vs. Time (guestroom or frontdoor): ")
    ...
    plot_eff_hum_vs_time(loc, show, write)